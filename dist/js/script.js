...
$(document).ready(function() {
    ...
    $('#login-frm').submit(function(e) {
            ...
            if ($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
                url: _fr_url_ + 'classes/Login.php?f=login',
                method: 'POST',
                data: $(this).serialize(),
                error: err => {
                    console.log(err)
                ...
                success: function(resp) {
                    if (resp) {
                        ...
                        if (resp.status == 'success') {
                            location.replace(_fr_url_ + 'admin');
    ...
    $('#flogin-frm').submit(function(e) {
        ...
        if ($('.err_msg').length > 0)
            $('.err_msg').remove()
        $.ajax({
            url: _fr_url_ + 'classes/Login.php?f=flogin',
            method: 'POST',
            data: $(this).serialize(),
            error: err => {
                console.log(err)
            ...
            success: function(resp) {
                if (resp) {
                    resp = JSON.parse(resp)
                    if (resp.status == 'success') {
                        location.replace(_fr_url_ + 'faculty');
                    } else if (resp.status == 'incorrect') {
                        var _frm = $('#flogin-frm')
                        var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>"
                        _frm.prepend(_msg)
    ...
    $('#slogin-frm').submit(function(e) {
            ...
            if ($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
                url: _fr_url_ + 'classes/Login.php?f=slogin',
                method: 'POST',
                data: $(this).serialize(),
                error: err => {
                    console.log(err)
                ...
                success: function(resp) {
                    if (resp) {
                        resp = JSON.parse(resp)
                        if (resp.status == 'success') {
                            location.replace(_fr_url_ + 'student');
                        } else if (resp.status == 'incorrect') {
                            var _frm = $('#slogin-frm')
                            var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>"
                            _frm.prepend(_msg)
    ...
    $('#system-frm').submit(function(e) {
        ...
        if ($('.err_msg').length > 0)
            $('.err_msg').remove()
        $.ajax({
            url: _fr_url_ + 'classes/SystemSettings.php?f=update_settings',
