...
Class Master extends DBConnection {
	...
	function save_post(){
		if(empty($_POST['id'])){
			...
			$prefix = date("Ymd");
			$code = sprintf("%'.04d", 1);
			if(!is_dir(sc_app."uploads/posts/"))
			mkdir(sc_app."uploads/posts/");

			while(true){
				if(is_dir(sc_app."uploads/posts/".$prefix.$code."/")){
					$code = sprintf("%'.04d", abs($code) + 1);
				}else{
					mkdir(sc_app."uploads/posts/".$prefix.$code."/");
		...
		if($save){
			...
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				$err='';
				if(!is_dir(sc_app.$file_path))
					mkdir(sc_app.$file_path);
				foreach($_FILES['img']['tmp_name'] as $k => $v){
					if(!empty($_FILES['img']['tmp_name'][$k])){
						...
						$spath = sc_app.$file_path.$_FILES['img']['name'][$k];
						$i = 1;
						while(true){
							if(is_file($spath)){
								$spath = sc_app.$file_path.($i++).'_'.$_FILES['img']['name'][$k];
							}else{
								break;
							}
						}
	...
	function delete_post(){
		...
		if($del){
			...
			if(is_dir(sc_app.$path)){
				$fopen = scandir(sc_app.$path);
				foreach($fopen as $file){
					if(!in_array($file, ['.', '..']) && is_file(sc_app.$path.$file)){
						unlink(sc_app.$path.$file);
					}
				}
				rmdir(sc_app.$path);
			}
