		...
		// if(!isset($_SESSION['system_info'])){
			...
			$qry = $this->conn->query($sql);
				while($row = $qry->fetch_assoc()){
					$_SESSION['system_info'][$row['si_meta_field']] = $row['si_meta_value'];
				}
		// }
	}
	function update_system_info(){
		$sql = "SELECT * FROM system_info Where syid=2";
		$qry = $this->conn->query($sql);
			while($row = $qry->fetch_assoc()){
				if(isset($_SESSION['system_info'][$row['si_meta_field']]))unset($_SESSION['system_info'][$row['si_meta_field']]);
				$_SESSION['system_info'][$row['si_meta_field']] = $row['si_meta_value'];
			}
		return true;
	}
	function update_settings_info(){
		...
		foreach ($_POST as $key => $value) {
			...
			if(isset($_SESSION['system_info'][$key])){
				...
				$qry = $this->conn->query("UPDATE system_info set si_meta_value = '{$value}' where syid=2 and si_meta_field = '{$key}' ");
			}else{
				$qry = $this->conn->query("INSERT into system_info set syid=2,si_meta_value = '{$value}', si_meta_field = '{$key}' ");
			}
		}
		...
		if(!empty($_FILES['img']['tmp_name'])){
			...
			if(is_file(sc_app.$fname))
			unlink(sc_app.$fname);

			$upload =imagepng($temp,sc_app.$fname,9);
			if($upload){
				if(isset($_SESSION['system_info']['logo'])){
					$qry = $this->conn->query("UPDATE system_info set si_meta_value = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where syid=2 and si_meta_field = 'logo' ");
					if(is_file(sc_app.$_SESSION['system_info']['logo'])) unlink(sc_app.$_SESSION['system_info']['logo']);
				}else{
					$qry = $this->conn->query("INSERT into system_info set syid=2, si_meta_value = '{$fname}',si_meta_field = 'logo' ");
				}
			}
			imagedestroy($temp);
		}
		if(!empty($_FILES['cover']['tmp_name'])){
			...
			if(is_file(sc_app.$fname))
			unlink(sc_app.$fname);
			$upload =imagepng($temp,sc_app.$fname,8);
			if($upload){
				if(isset($_SESSION['system_info']['cover'])){
					$qry = $this->conn->query("UPDATE system_info set si_meta_value = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where syid=2, si_meta_field = 'cover' ");
					if(is_file(sc_app.$_SESSION['system_info']['cover'])) unlink(sc_app.$_SESSION['system_info']['cover']);
				}else{
					$qry = $this->conn->query("INSERT into system_info set syid=2, si_meta_value = '{$fname}',si_meta_field = 'cover' ");
				}
			}
			imagedestroy($temp);
		}
		if(isset($_FILES['banners']) && count($_FILES['banners']['tmp_name']) > 0){
			...
			if(!is_dir(sc_app.$banner_path))
				mkdir(sc_app.$banner_path);
			foreach($_FILES['banners']['tmp_name'] as $k => $v){
				if(!empty($_FILES['banners']['tmp_name'][$k])){
					...
					$temp = imagescale($uploadfile,$width,$height);
					$spath = sc_app.$banner_path.'/'.$_FILES['banners']['name'][$k];
					$i = 1;
					while(true){
						if(is_file($spath)){
							$spath = sc_app.$banner_path.'/'.($i++).'_'.$_FILES['banners']['name'][$k];
						}else{
							break;
						}
					}
