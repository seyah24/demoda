...
Class Users extends DBConnection {
		...
		if(empty($id)){
			...
			if($qry){
				...
				if(!empty($_FILES['img']['tmp_name'])){
					if(!is_dir(sc_app."uploads/avatars"))
						mkdir(sc_app."uploads/avatars");
					...
					unlink(sc_app.$fname);
					$upload =imagepng($temp,sc_app.$fname);
					if($upload){
	...
	public function delete_users(){
		...
		if($qry){
			...
			if(is_file(sc_app."uploads/avatars/$id.png"))
				unlink(sc_app."uploads/avatars/$id.png");
			return 1;
		}else{
			return false;
		}
	...
	public function save_member(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				if(!empty($_FILES['img']['tmp_name'])){
					if(!is_dir(sc_app."uploads/member"))
						mkdir(sc_app."uploads/member");
					$ext = pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);
	...
	public function delete_member(){
		...
		if($qry){
			...
			if(is_file(sc_app."uploads/member/$id.png"))
				unlink(sc_app."uploads/member/$id.png");
			return 1;
		}else{
			return false;
		}
	...
	function update_user_meta(){
		...
		if($save){
			...
			if(isset($_FILES['vaccine_card_path']) && !empty($_FILES['vaccine_card_path']['tmp_name'])){
				...
				if(!is_dir(sc_app."uploads/vaccines/"))
					mkdir(sc_app."uploads/vaccines/");
				...
				if(!in_array($type, ['image/jpeg', 'image/png'])){
					...
					if(is_file(sc_app.$filename))
						unlink(sc_app.$filename);
					$upload = imagepng($img, sc_app.$filename, 6);
					if($upload){
