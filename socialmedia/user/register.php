...
<!-- jQuery -->
<script src="<?= sc_url ?>plugins/jquery/jquery.min.js"></script>
<!-- Bootstrap 4 -->
<script src="<?= sc_url ?>plugins/bootstrap/js/bootstrap.bundle.min.js"></script>
<!-- AdminLTE App -->
<script src="<?= sc_url ?>dist/js/adminlte.min.js"></script>
...
function displayImg(input,_this) {
  ...
  $(document).ready(function(){
    ...
    $('#register-form').submit(function(e){
        ...
        start_loader()
        $.ajax({
            url:_sc_url_+"classes/Users.php?f=registration",
            method:'POST',
            type:'POST',
            ...
            success:function(resp){
                if(resp.status == 'success'){
                    location.replace(_sc_url_+'user/?page=user/profile')
                }else if(!!resp.msg){
                    el.html(resp.msg)
                    el.show('slow')
                    _this.prepend(el)
