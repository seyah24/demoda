                    ...
                    <div class="d-flex align-items-center">
                      <div class="btn-group nav-link text-reset">
                        ...
                        <div class="dropdown-menu" role="menu">
                          <a class="dropdown-item" href="<?php echo sc_url.'user/?page=user' ?>"><span class="fa fa-user"></span> My Account</a>
                          <div class="dropdown-divider"></div>
                          <a class="dropdown-item" href="<?php echo sc_url.'/classes/Login.php?f=user_logout' ?>"><span class="fas fa-sign-out-alt"></span> Logout</a>
                        </div>
                    </div>
                    </div>
                </div>
  ...
  $(function(){
    $('#search').on('input change', function(){
      ...
      if(kw == ''){
        ...
        process_ajax.abort()
        process_ajax = $.ajax({
          url:_sc_url_+"classes/Master.php?f=search_user",
          method:'POST',
          data:{search:kw},
          dataType:'json',
          error:err=>{
            ...
            if(resp.status == 'success'){
              if(Object.keys(resp.data).length > 0){
                Object.keys(resp.data).map(k=>{
                  ...
                  anchor.attr('href', _sc_url_+"user/?page=user/profile&user_id="+user.id)
