		...
		<div class="card-body">
			...
			<!-- <div class="form-group">
				<label for="" class="control-label">Welcome Content</label>
	             <textarea name="content[welcome]" id="" cols="30" rows="2" class="form-control summernote"><?php echo  is_file(sc_app.'welcome.html') ? file_get_contents(sc_app.'welcome.html') : "" ?></textarea>
			</div>
			<div class="form-group">
				<label for="" class="control-label">About Us</label>
	             <textarea name="content[about]" id="" cols="30" rows="2" class="form-control summernote"><?php echo  is_file(sc_app.'about.html') ? file_get_contents(sc_app.'about.html') : "" ?></textarea>
			</div> -->
			<div class="form-group">
            ...
            if(is_dir(sc_app.$upload_path)): 
			$file= scandir(sc_app.$upload_path);
                foreach($file as $img):
            ...
            ?>
                <div class="d-flex w-100 align-items-center img-item">
                    <span><img src="<?php echo sc_url.$upload_path.'/'.$img."?v=".(time()) ?>" width="150px" height="100px" style="object-fit:cover;" class="img-thumbnail" alt=""></span>
                    <span class="ml-4"><button class="btn btn-sm btn-default text-danger rem_img" type="button" data-path="<?php echo sc_app.$upload_path.'/'.$img ?>"><i class="fa fa-trash"></i></button></span>
                </div>
            <?php endforeach; ?>
            <?php endif; ?>
			</form>
...
<script>
	...
	function delete_img($path){
        ...
        $.ajax({
            url: _sc_url_+'classes/Master.php?f=delete_img',
            data:{path:$path},
            method:'POST',
            dataType:"json",
            error:err=>{
