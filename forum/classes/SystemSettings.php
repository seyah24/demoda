	...
	function update_settings_info(){
		...
		foreach ($_POST as $key => $value) {
			...
			if(isset($_SESSION['system_info'][$key])){
				...
				$qry = $this->conn->query("UPDATE system_info set si_meta_value = '{$value}' where syid=1 and si_meta_field = '{$key}' ");
			}else{
				$qry = $this->conn->query("INSERT into system_info set syid= 1, si_meta_value = '{$value}', si_meta_field = '{$key}' ");
			}
		}
		...
		if(!empty($_FILES['img']['tmp_name'])){
			...
			if($upload){
				if(isset($_SESSION['system_info']['logo'])){
					$qry = $this->conn->query("UPDATE system_info set si_meta_value = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where syid=1 and si_meta_field = 'logo' ");
					if(is_file(base_app.$_SESSION['system_info']['logo'])) unlink(base_app.$_SESSION['system_info']['logo']);
				}else{
					$qry = $this->conn->query("INSERT into system_info set syid=1, si_meta_value = '{$fname}',si_meta_field = 'logo' ");
				}
			}
			imagedestroy($temp);
		}
		if(!empty($_FILES['cover']['tmp_name'])){
			...
			if($upload){
				if(isset($_SESSION['system_info']['cover'])){
					$qry = $this->conn->query("UPDATE system_info set si_meta_value = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where syid=1 and si_meta_field = 'cover' ");
					if(is_file(base_app.$_SESSION['system_info']['cover'])) unlink(base_app.$_SESSION['system_info']['cover']);
				}else{
					$qry = $this->conn->query("INSERT into system_info set syid=1, si_meta_value = '{$fname}',si_meta_field = 'cover' ");
				}
			}
			imagedestroy($temp);
		}
