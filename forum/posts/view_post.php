<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT topics.*, username, avatar, cate_name as `category` FROM topics inner join category_list on topics.cateid = category_list.cateid inner join `users` on userid = uid where toid= '{$_GET['id']}'");
    if($qry->num_rows > 0){
    ...
    <div class="container">
        <div class="card rounded-0 shadow">
            <div class="card-header">
                ...
                <?php if($_settings->userdata('id') == $userid): ?>
            ...
            <div class="card-body">
                <div class="contrain-fluid">
                    ...
                    <div style="line-height:1em" class="mb-3">
                        <h2 class="font-weight-bold mb-0 border-bottom"><?= $to_title ?></h2>
                        <div class="py-1">
                    ...
                    <div class="list-group comment-list mb-3 rounded-0">
                        <?php 
                        $comments = $conn->query("SELECT topic_com.*, username, avatar FROM topic_com inner join users on userid = uid where topic_com.toid ='{$toid}' order by abs(unix_timestamp(tc_date_created)) asc ");
                            ...
                            <div class="d-flex align-items-center w-100">
                                ...
                                <div class="col-auto flex-shrink-1 flex-grow-1">
                                    <div style="line-height:1em">
                                        ...
                                        <div><small class="text-muted"><i><?= date("Y-m-d h:i a", strtotime($row['tc_date_created'])) ?></i></small></div>
                                    </div>
                                </div>
                                <?php if($row['userid'] == $_settings->userdata('id')): ?>
                                    <a href="javascript:void(0)" class="text-danger text-decoration-none delete-comment" data-id = '<?= $row['userid'] ?>'><i class="fa fa-trash"></i></a>
                                <?php endif; ?>
                            </div>
                            <hr>
                            <div><?= $row['topic_comment'] ?></div>
                        </div>
                        <?php endwhile; ?>
                    </div>
