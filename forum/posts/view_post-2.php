...
<script>
    $(function(){
        $('.delete-comment').click(function(){
            _conf("Bạn muốn xoá bình luận này đúng không?", "delete_comment", [$(this).attr('data-id')])
        })
        $('#delete_post').click(function(){
            _conf("Bạn muốn xoá bài viết này đúng không?", "delete_post", ['<?= isset($id) ? $id : '' ?>'])
        })
    ...
    function delete_post($id){
		...
		$.ajax({
			...
			error:err=>{
				console.log(err)
				alert_toast("ID:010-deletepost/forum/posts.",'error');
				end_loader();
			},
			success:function(resp){
				if(typeof resp == 'object' && resp.status == 'success'){
					...
					alert_toast("ID:011-deletepost/forum/posts.",'error');
					end_loader();
				}
			}
		})
    ...
    function delete_comment($id){
		...
		$.ajax({
			...
			error:err=>{
				console.log(err)
				alert_toast("ID:012-deletecomment/forum/posts.",'error');
				end_loader();
			},
			success:function(resp){
				if(typeof resp == 'object' && resp.status == 'success'){
					...
					alert_toast("ID:013-deletecomment/forum/posts.",'error');
					end_loader();
				}
			}
		})
